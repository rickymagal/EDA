...
int ig(TG *g1, TG *g2){
	TG * atual1 = g1;
	TG * atual2 = g2;
	if(atual1->id_no!=atual2->id_no) return 0;
	while(atual1!=NULL && atual2!=NULL){
		TVIZ * temp1 = atual1->prim_viz;
		TVIZ * temp2 = atual2->prim_viz;
		while(temp1!=NULL && temp2!=NULL){
			if(temp1->id_viz!=temp2->id_viz) return 0;
			temp1 = temp1->prox_viz;
			temp2 = temp2->prox_viz;
		}
		atual1 = atual1->prox_no;
		atual2 = atual2->prox_no;
	}return 1;
}

/*
int nao_tem_mesma_cor(TG *g){
	TVIZ * atual = g->prim_viz;
	while(atual!=NULL){
		TG * temp = TG_busca_no(g,atual->id_viz);
		if(temp->cor!=g->cor) return 1;
